  ...
  validateAccount = async () => {
    ...
    return window.web3.personal.sign(
      window.web3.fromUtf8(
        `Hi there from Proof of Existence DApp! To connect to the Dapp, sign this message to prove you have access to this account. 
      ...
      (error, signature) => {
        if (error) {
          ...
          window.toastProvider.addMessage(successMessage, {
            variant: "success",
            secondaryMessage: "Welcome to the Proof Of Existence App 🎉"
          });
  ...
  contractMethodSendWrapper = (contractMethod, argument, callback) => {
    ...
    if (!this.state.account) {
      this.openTransactionConnectionModal(null, () => {
        ...
        this.contractMethodSendWrapper(contractMethod, argument, callback);
      });
      return;
    }
    ...
    try {
      contract.methods.notarize(argument)
        ...
        .on("confirmation", (confirmationNumber, receipt) => {
          ...
          } else if (transaction.confirmationCount === 4) {
        ...
        .on("error", error => {
          ...
          window.toastProvider.addMessage("Document not notarized...", {
            secondaryMessage: "Could not notarize document.",
            actionHref: "",
            actionText: "",
            variant: "failure"
          });
